...
FROM
    INPUT FULL
    OUTER JOIN PW5000_PANEL_I ON INPUT.ID = PW5000_PANEL_I.ID FULL
    OUTER JOIN LOGICAL_DEV_D ON LOGICAL_DEV_D.HARDWARE_ID = INPUT.ID FULL
    OUTER JOIN PANEL ON PANEL.ID = LEFT(INPUT.ID, 17) FULL
    OUTER JOIN PW5000_PANEL_M ON LEFT(INPUT.ID, 21) = PW5000_PANEL_M.ID
WHERE
